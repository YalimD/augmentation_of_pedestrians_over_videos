...
namespace RockVR.Video.Editor
{
    ...
    public class VideoCaptureEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            videoCapture.mode = (VideoCaptureBase.ModeType)EditorGUILayout.EnumPopup("Mode", videoCapture.mode);
            ...
            videoCapture.format = (VideoCaptureBase.FormatType)EditorGUILayout.EnumPopup("Format", videoCapture.format);
            if (videoCapture.format == VideoCapture.FormatType.NORMAL)
            {
                if (videoCapture.isDedicated)
                {
                    videoCapture.frameSize = (VideoCaptureBase.FrameSizeType)EditorGUILayout.EnumPopup("Frame Size", videoCapture.frameSize);
                }
            }
            else if (videoCapture.format == VideoCapture.FormatType.PANORAMA)
            {
                ...
                videoCapture.panoramaProjection = (VideoCaptureBase.PanoramaProjectionType)EditorGUILayout.EnumPopup("Projection Type", videoCapture.panoramaProjection);
                if (videoCapture.panoramaProjection == VideoCapture.PanoramaProjectionType.EQUIRECTANGULAR)
                {
                    videoCapture.frameSize = (VideoCaptureBase.FrameSizeType)EditorGUILayout.EnumPopup("Frame Size", videoCapture.frameSize);
                }
                videoCapture._cubemapSize = (VideoCaptureBase.CubemapSizeType)EditorGUILayout.EnumPopup("Cubemap Size", videoCapture._cubemapSize);
                videoCapture.captureGUI = EditorGUILayout.Toggle("Capture GUI", videoCapture.captureGUI);
                GUILayout.EndVertical();
            }
            videoCapture.stereo = (VideoCaptureBase.StereoType)EditorGUILayout.EnumPopup("Stereo Format", videoCapture.stereo);
            if (videoCapture.stereo != VideoCapture.StereoType.NONE)
            {
                GUILayout.BeginVertical("box");
                GUILayout.Label("Stereo Format");
                videoCapture.stereoFormat = (VideoCaptureBase.StereoFormatType)EditorGUILayout.EnumPopup("Stereo Format Type", videoCapture.stereoFormat);
            ...
            videoCapture.encodeQuality = (VideoCaptureBase.EncodeQualityType)EditorGUILayout.EnumPopup("Encode Quality", videoCapture.encodeQuality);
            videoCapture._antiAliasing = (VideoCaptureBase.AntiAliasingType)EditorGUILayout.EnumPopup("Anti Aliasing", videoCapture._antiAliasing);
            videoCapture._targetFramerate = (VideoCaptureBase.TargetFramerateType)EditorGUILayout.EnumPopup("Target FrameRate", videoCapture._targetFramerate);
